                 ...
                 (let ([lvs (get-values-eval a env s-f)])
                   (let ([update (update-store-bind-symbols (closV-arg v-f) 
                     ...
                     (eval (closV-body v-f)  
                           (env*s-env update) 
                           (env*s-store update))))])]
    [recC (name body value) 
         (cond [(check-if-reserved-symbol name) (error 'recC "error")]
               [else (set-up-rec name body value env store)])]))
 
; Evaluates a recursive call
(define (set-up-rec [name : symbol] 
                    [value : ExprC] 
                    [body : ExprC] 
                    [env : Env] 
                    [store : Store]) : Result
  (let 
      ([nextLocation (add1 (get-last-location (cond [(empty? store) 
                                                     (list (cell -1 (numV -1)))] 
                                                    [else store])))])
       (let ([extendStore (n*s-store (allocate store 1 (numV 42)))]
             [extendEnv (cons (bind name nextLocation) env)])
    (type-case Result (eval value extendEnv extendStore)
      [v*s (v-value s-value)
           (let ([updated-store (update-store nextLocation v-value s-value)])
             (type-case Result (eval body extendEnv updated-store)
               [v*s (v-body s-body)
                    (v*s v-body s-body)]))]))))
...
; creates the class s expression to be parse
(define (create-class-s-expression [class : symbol]
                                   [parent : symbol]
                                   [fields : (listof s-expression)] 
                                   [methodNames : (listof s-expression)] 
                                   [methodBodies : (listof s-expression)]) : s-expression
  (let ([ifChain (build-if-chain methodNames methodBodies)]
...
(define (build-if-chain [methodNames : (listof s-expression)]  
  ...
  (cond [(eq? (length methodNames) 1) `{if {eq? mesg ,(string->s-exp 
                                     ...
                                     {fn {this} ,(first methodBodies)} 
                                     {parent mesg}}]
        [else `{if {eq? mesg ,(string->s-exp 
                               (symbol->string 
                                (s-exp->symbol (first methodNames))))}
                   {fn {this} ,(first methodBodies)}
                   ,(build-if-chain (rest methodNames) (rest methodBodies))}]))
...
(define (get-method-details [s :  (listof s-expression)] 
  ...
  (cond [(or (empty? s) (empty? (s-exp->list(first s))))  empty]
...
(define (top-eval [program : s-expression]) : string
  (serialize (eval (parse (create-final-program (s-exp->list program))) empty empty)))
...
(define (serialize [e : Result]) : string
  (cond [(numV? (v*s-v e)) (to-string (numV-n (v*s-v e)))]
        ...
        [(stringV? (v*s-v e)) (stringV-s (v*s-v e))]))
...
(top-eval '{{class Point extends Object
              {x y}
              {get-x {e} {+ x e}}
                     {get-y {} y}
...
(test/exn (top-eval '{{fn {} 9} 17}) "I think")
...
(test/exn (update-store-bind-symbols (list 'a 'b 'c) 
                                     (list (numV 3)) 
                                     (list (bind 'a 0)) 
                                     (list (cell 0 (numV 5)))) "not")
...
(test (eval (parse '"string") empty empty) (v*s (stringV "string") empty))
